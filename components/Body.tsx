...
{show &&  <Container className="rounded-xl  w-60 h-50  -translate-y-15 translate-x-8">
  <div className="flex justify-between items-center px-4 pt-2">
    <Question className="text-lg font-bold">BookMark</Question>
            <Question className="text-neutral-400 text-xs">3 new</Question></div>
              
            <Data className="bg-blue-600" name="React hooks" time="12:27 am" key={1}></Data>
            <Data className="bg-gray-700" name="Promises in JS" time="1:27 am" key={3}></Data> 
              <Data  name="Debouncing" time="12:27 pm" key={2}></Data>

            <Question className="text-neutral-400 text-xs flex justify-center pt-2 border-t-2 border-neutral-300">View All Bookmarks</Question>
          ...
          <Container className="rounded-lg p-4 bg-neutral-100"> 
           ...
           <Container className="ml-49 p-4 rounded-lg bg-neutral-100 border-2 border-cyan-500">
            <Titles title={'Smarter Learning, Faster Results'}>
             ...
             <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="rgb(0, 183, 219)" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-cyan-500  translate-x-75 translate-y-2 lucide lucide-mouse-pointer2-icon lucide-mouse-pointer-2"><path d="M4.037 4.688a.495.495 0 0 1 .651-.651l16 6.5a.5.5 0 0 1-.063.947l-6.124 1.58a2 2 0 0 0-1.438 1.435l-1.579 6.126a.5.5 0 0 1-.947.063z"/></svg>
          </Container>
...
function Data({time,name,className} : {className ?: string,time : string,name:string}){
  return(<div className="flex px-4 items-center justify-between">
       <div className="flex items-center gap-1 my-2">
    <div className={cn(`w-7 h-7 rounded-full bg-amber-600`,className)}>
    </div>
...
export function Container({ children , className}: {className ?: string,children : React.ReactNode}) {
  return (
    <div className={cn(`w-90 h-33  m-4 text-center  shadow-input bg-neutral-200`,className)}>
...
export function Question({
  ...
  return (
    <div className={cn(`text-2xl font-display font-semibold flex items-center justify-between`,className)}>
      {children}
    </div>
  );
}
